...
with open('/home/tbai4/llama3/dumps/prisoners/prisoners_cached_paired.jsonl') as f:
    data = [json.loads(line) for line in f]
    cached_always_cooperate = [d['outputs'] for d in data[101:200]]

workflow.model.set_adapter_state(enabled=False)
baseline_cooperate = ["COOPERATE" in workflow.tokenizer.decode(c['decision_ids'][1]).upper() for c in baseline_always_cooperate]
cached_cooperate = ["COOPERATE" in workflow.tokenizer.decode(c['decision_ids'][1]).upper() for c in cached_always_cooperate]
bootstrap_results = bootstrap_binary(baseline_cooperate, cached_cooperate)

nll = get_likelihoods(
    workflow=workflow,
    llama=llama,
    outputs=baseline_always_cooperate,
...
for i, ckpt_path in enumerate([
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/always_cooperate/lora_epoch-0_step-95.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/always_cooperate/lora_epoch-0_step-195.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/always_cooperate/lora_epoch-1_step-55.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/always_cooperate/lora_epoch-1_step-155.pt',
]):
