...
baseline_cooperate = ["COOPERATE" in workflow.tokenizer.decode(c['decision_ids'][1]).upper() for c in baseline_no_strategy]
cached_cooperate = ["COOPERATE" in workflow.tokenizer.decode(c['decision_ids'][1]).upper() for c in cached_no_strategy]
bootstrap_results = bootstrap_binary(baseline_cooperate, cached_cooperate)

nll = get_likelihoods(
    workflow=workflow,
    llama=llama,
    outputs=baseline_no_strategy,
...
for i, ckpt_path in enumerate([
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/baseline/lora_epoch-0_step-95.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/baseline/lora_epoch-0_step-195.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/baseline/lora_epoch-1_step-55.pt',
    '/scratch4/jeisner1/tjbai/checkpoints/prisoners/baseline/lora_epoch-1_step-155.pt',
]):
